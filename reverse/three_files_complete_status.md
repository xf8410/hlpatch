...
| 段 | 大小 | 状态 |
|----|------|------|
| methods | 12.1MB | ⚠️ 已定位get_Speed等方法, 但entry_size不固定(v31变更), 完整解析需SO Type索引表 |
| parameters | 3.7MB | ⚠️ 依赖methods段的parameterStart索引 |
...
| single_mode_14_* | 16张表 | 拉面杯完整数据 (98地区效果/180奖励/21检查点等) |
| text_data | 95,128 | 全部文本数据 |

### ✅ 导出

- `reverse/mdb_key_tables_export.json` (4.4MB) — 14张关键表完整导出

### ⚠️ 还有数百张表

- 619张总表中已导出14张关键表
- 剩余约600张: announce/campaign/champions/chara/collect/crane/story/skill/succession/team等
- 大部分是UI/活动/比赛相关, 对训练逆向帮助不大
