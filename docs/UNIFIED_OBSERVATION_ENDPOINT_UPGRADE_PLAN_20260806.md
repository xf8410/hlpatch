...
一次构建补齐当前逆向任务反复卡住的通用能力，覆盖：请求/响应、IL2CPP 精确调用链、育成训练公式、综合评价、技能、支援卡、育成角色、继承种马相性与比赛相性。该计划优先增加可复用的底层能力，不为每个页面重复写一次性端点。
...
按ID读取单条完整交换；大正文写入插件私有持久文件并返回可完整下载的文件端点、长度、SHA-256。内存缓冲重启丢失时可继续读取历史记录。
...
这能读取**角色组合的基础相性数字**，但不能单靠角色ID代表某一匹具体种马的完整继承相性。
...
这是验证“比赛相性到底加多少”的关键端点。对继承树每个节点读取完整比赛历史，并与比赛关系Master逐项连接，输出：
...
读取当前种马选择页面或育成开始请求中的**具体种马对象**，返回服务器已经提供的相性总数字、圈级、各分项（若DTO有）、亲本/祖辈ID和本地重算差异。因此：如果游戏对象或响应中存在该数字，SO可以直接读取；若只存在圈标，SO仍可用Master+战绩重算，但必须标记 `computed_local`，不能冒充服务器字段。
...
用于确认服务器何时计算相性、是否对候选列表下发精确数字，以及刷新/筛选是否改变结果。

## P1：比赛观测

### 30. `/race/pre_snapshot` 与 `/race/result_trace`
...
B. call_targets/callers + 精确Hook管理
C. exchange_id协议全链与持久交换记录
D. snapshot/transition/timeline通用关联框架
E. evaluation/skills/training专用适配
F. support/character/event身份层
G. inherit tree/pair/full/race/selected_parent trace
H. race trace与稳定性回归
```

A-D完成后，后续大多数机制不再需要新增底层端点，只需增加类型适配器。
...
8. 协议：重启前捕获的大交换仍可按ID完整读取，且dropped_count可核对。
9. 版本：游戏更新后旧Hook显示signature_mismatch或unresolved，不静默使用旧地址。
10. 稳定性：所有单目标查询均不依赖 `/il2cpp/classes`、`/il2cpp/disassemble_addr*` 或全量方法dump。

## 当前事实边界

- 当前已部署SO在本次规划时不可连接，故未读取实时 `/inherit/compat` 输出；不可把已有端点名视为精确相性已经闭合。
- 仓库 `reverse/relation_formula_exact.md` 保存社区公式与MDB群组解释，但“比赛相性每项加多少”、当前版本完整树修正和服务器是否下发精确数字仍未由运行时Hook确认。
- `hlpatch_endpoints.txt` 已列出142个端点；本计划不是继续堆大量重复页面端点，而是先补精确MethodInfo、统一交换记录和时序关联三项基础设施。
